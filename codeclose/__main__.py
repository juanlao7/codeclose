...
def addSizeArguments(parser):
    parser.add_argument('--license-id-size', default=model.DEFAULT_LICENSE_ID_SIZE, type=UnsignedInt, help='Size, in bits, of the license ID field. {} by default ({:,} possible ids).'.format(model.DEFAULT_LICENSE_ID_SIZE, 2 ** model.DEFAULT_LICENSE_ID_SIZE))
    parser.add_argument('--product-id-size', default=model.DEFAULT_PRODUCT_ID_SIZE, type=UnsignedInt, help='Size, in bits, of the product ID field. {} by default ({:,} possible ids).'.format(model.DEFAULT_PRODUCT_ID_SIZE, 2 ** model.DEFAULT_PRODUCT_ID_SIZE))
    parser.add_argument('--expiration-time-size', default=model.DEFAULT_EXPIRATION_TIME_SIZE, type=UnsignedInt, help='Size, in bits, of the expiration time field. {} by default (maximum expiration date: 36812-02-19 16:53 UTC).'.format(model.DEFAULT_EXPIRATION_TIME_SIZE))     # Too big for datetime.fromtimestamp.
    parser.add_argument('--hash-size', default=model.DEFAULT_HASH_SIZE, type=UnsignedInt, help='Size, in bytes, of the hash needed for validating the product key.')
...
class Commands(object):
    ...
    def generateSigningKeys(cls):
        def preparer(parser):
            ...
            parser.add_argument('--size', default=model.DEFAULT_RSA_KEY_SIZE, type=UnsignedInt, help='RSA key size, in bits. %s by default.' % model.DEFAULT_RSA_KEY_SIZE)
            parser.add_argument('--public-exponent', default=model.DEFAULT_RSA_PUBLIC_EXPONENT, type=int, help='RSA public exponent. %s by default.' % model.DEFAULT_RSA_PUBLIC_EXPONENT)
    ...
    def generateEncryptingKey(cls):
        def preparer(parser):
            def handler(args):
                args.key_path.write(model.generateEncryptingKey(args.size))
            
            parser.add_argument('--size', default=model.DEFAULT_AES_KEY_SIZE, type=int, choices=[128, 192, 256], help='AES key size, in bits (128, 192 or 256). %s by default.' % model.DEFAULT_AES_KEY_SIZE)
            parser.add_argument('key_path', type=argparse.FileType('wb'), help='File path where the AES key for encrypting and decrypting source code will be stored.')
            return handler
    ...
    def protect(cls):
        def preparer(parser):
            def handler(args):
                model.protect(args.key_path.read(), args.dest_directory, args.src, args.encryption_excluded, args.follow_symlinks)
            ...
            parser.add_argument('key_path', type=argparse.FileType('rb'), help='File containing the AES key for encrypting and decrypting source code.')
    ...
    def createProductKey(cls):
        def preparer(parser):
            def handler(args):
                print(model.createProductKey(args.key_path.read(), args.license_id, args.product_id, args.expiration_time, args.groups_length, args.license_id_size, args.product_id_size, args.expiration_time_size, args.hash_size))
    ...
    def readProductKey(cls):
        def preparer(parser):
            def handler(args):
                try:
                    licenseId, productId, expirationTime = model.readProductKey(args.key_path.read(), args.product_key, args.license_id_size, args.product_id_size, args.expiration_time_size, args.hash_size)
    ...
    @classmethod
    def computeResponse(cls):
        def preparer(parser):
            def handler(args):
                pass
            
            addSizeArguments(parser)
            parser.add_argument('private_key_path', type=argparse.FileType('rb'), help='File containing the RSA public key for reading and verifying product keys.')
            parser.add_argument('code_key_path', type=argparse.FileType('rb'), help='File containing the AES key for encrypting and decrypting source code.')
            parser.add_argument('product_key', type=str, help='The product key to verify and read.')
            return handler
